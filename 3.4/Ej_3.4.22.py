...
# Ej 21
indices_numeros = df.select_dtypes(exclude=[object])

df_nuevo = df.drop(columns=indices_numeros)
...
for clave, valor in df_dict.items():
    if (valor == object) and (clave != 'Cobrado'):
        df_media.append(clave)

df_nuevo = df_nuevo.drop(columns=df_media)
media = round(df_nuevo.groupby("Cobrado").mean(), 2)
